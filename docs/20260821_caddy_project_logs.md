...
Let an authenticated consumer view bounded, privacy-filtered Caddy access logs for every active Caddy project they are authorized to read, without exposing Caddy's admin API, arbitrary files, another project's traffic, or unbounded log streams.
...
- Stored records exclude request/response headers and query strings. Client IPv4/IPv6 addresses are masked to `/24` and `/64`; Caddy credentials logging remains disabled.
...
- The API returns only a typed field allowlist: timestamp, method, host, path, status, duration, response bytes, and masked client network.
...
- Callers never submit filesystem paths or logger IDs. Opaque, versioned cursors are strictly validated and expire safely when rotation invalidates their source.
...
- Add an `access-log` bounded context with a filesystem source, Caddy JSON parser, cursor codec, list use case, and typed results. Bound record count, line size, scanned/decompressed bytes, and malformed-line handling; reject symlinks and non-regular files.
...
- Provision the log root for the Caddy service with restrictive ownership/modes. Run Caddy validation with Caddy-compatible filesystem permissions so validation cannot create root-owned log files.
...
- Roll out behind the log-root environment setting: native Caddy validation and privacy fixtures first, then a one-project staging canary, then production enablement.
...
- [x] 2. Generate per-project named Caddy loggers, host mappings, privacy filters, rotation settings, and default-logger exclusions; cover active, disabled, catalog-only, multi-domain, and collision cases with snapshots plus native Caddy validation.
- [x] 3. Implement the bounded file source, JSON-to-public-record parser, gzip archive reading, reverse chronological paging, opaque cursor codec, resource limits, symlink defenses, and rotation-race results with filesystem fixtures.
...
- [x] 6. Add the project-detail access-log client and panel using reusable `#ui` components; test paging, polling cleanup, refresh/pause, visibility changes, and all empty/error states, then verify the rendered flow in a browser.
...
- [x] 8. Update systemd/migration installation and environment templates to create the Caddy-owned `0700` log root with `0600` files, validate as the service user, and document capacity/retention and the opt-in rollout switch.
- [ ] 9. Add an end-to-end staging check that sends requests containing query secrets, cookies, authorization headers, and distinct project hosts; prove isolation, redaction, permissions, rotation continuity, HTTP/Unix authorization parity, and bounded disk/read behavior before enabling production.
