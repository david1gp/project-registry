...
export async function visibleUserDirectoryList(
  ...
  if (actor.role === "own") {
    ...
    if (!actorR.success) return createResultError(op, "user directory is unavailable")
    return createResult([])
  }
  ...
  if (
    ...
    return createResultError(op, "user directory is unavailable")
  }
  try {
    ...
    if (usersR.data.data.length > maxUsers) return createResultError(op, "user directory is too large")
    ...
    for (const user of usersR.data.data) {
      if (
        ...
        return createResultError(op, "user directory contains an invalid mapping")
      }
      lookupCount += 1
      if (lookupCount > maxLookupCount) return createResultError(op, "user directory lookup limit exceeded")
      const usernameR = await preferredUsernameMap(user.preferredUsername, options.posixUsers, options)
      if (!usernameR.success) return createResultError(op, "user directory contains an invalid mapping")
      lookupCount += 1
      if (lookupCount > maxLookupCount) return createResultError(op, "user directory lookup limit exceeded")
      const roleR = await userRoleResolve(user.subject, options.accessToken, options.identityDirectory, options)
      if (!roleR.success) return createResultError(op, "user directory role lookup is unavailable")
    ...
    if (!visibleR.success) return createResultError(op, "user directory is unavailable")
